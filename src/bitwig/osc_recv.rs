use crate::bitwig::message::{
    BitwigMessage, ClipEvent, ClipMessage, DeviceMessage, TrackEvent, TrackMessage,
};
...
impl OscRecv {
            ...
            match sock.recv_from(&mut buf) {
                Ok((size, _addr)) => {
                    ...
                    self.handle_packet(packet)
                        .expect("Could not process OSC packet");
                }
                Err(e) => {
    ...
    fn handle_packet(&self, packet: OscPacket) -> anyhow::Result<()> {
        match packet {
    ...
    fn handle_bundle(&self, bundle: OscBundle) -> anyhow::Result<()> {
        for packet in bundle.content.into_iter() {
            self.handle_packet(packet)?
        }
    ...
    #[tracing::instrument]
    fn handle_message(&self, msg: OscMessage) -> anyhow::Result<()> {
    ...
    fn handle_primary_message(&self, msg: OscMessage) -> anyhow::Result<()> {
        match msg.addr.as_str() {
            s if s.ends_with("value") => {
                let param = Self::parse_param(&msg);
                self.tx.send(BitwigMessage::Primary(DeviceMessage::new(
                    param,
                    Self::arg_to_int(&msg.args[0]),
                )))?;
            }
            _ => {}
        }
    ...
    fn handle_track_message(&self, msg: OscMessage) -> anyhow::Result<()> {
        match msg.addr.as_str() {
            s if s.ends_with("selected") => {
                let track = Self::parse_track(&msg);
                let active = Self::arg_to_bool(&msg.args[0]);
                self.tx.send(BitwigMessage::Track(TrackMessage::new(
                    track,
                    active,
                    TrackEvent::Selected,
                )))?;
    ...
    fn handle_clip_message(&self, msg: OscMessage) -> anyhow::Result<()> {
        let (track, scene) = Self::parse_track_and_scene(&msg);
        let active = Self::arg_to_bool(&msg.args[0]);
        match msg.addr.as_str() {
            s if s.ends_with("isPlayingQueued") => {
                self.tx.send(BitwigMessage::Clip(ClipMessage::new(
                    track,
                    scene,
                    active,
                    ClipEvent::Queued,
    ...
    fn parse_param(msg: &OscMessage) -> u8 {
        ...
        let param = parts[3]
            .parse()
            .unwrap_or_else(|_| panic!("{:?}", msg.addr));
        param
    }
